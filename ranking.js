...
// --- CAMPEAO CAINDO
                ...
                mapaExtra.forEach(m => {
                    if (m.tipo === 'final') {
                        ...
                        // Usamos o operador ?. para evitar o erro de 'undefined' e || [] para garantir um array vazio
                        const finalistas = statusPaises?.finalistas || [];
                        const eliminados = statusPaises?.eliminados || [];
                        const disputa34 = statusPaises?.disputa34 || [];

                        const ehFinalista = finalistas.includes(palpiteID);
                        const ehEliminado = eliminados.includes(palpiteID);
                        const ehDisputa34 = disputa34.includes(palpiteID);
                        ...
                        if (colunasEliminaveis.includes(m.db)) {
                            // Trava para 3º e 4º lugar
                            if (['final_terceiro', 'final_quarto'].includes(m.db) && ehFinalista) {
                                status = 'ERROU';
                            } 
                            // Trava para Campeão e Vice
                            else if (['final_campeao', 'final_vice'].includes(m.db) && ehDisputa34) {
                                status = 'ERROU';
                            }
                            // Lógica original de eliminado
                            else if (isEliminado) {
                                status = 'ERROU';
                            }
                            // Lógica de Gabarito
                            else if (gabaritoID != null && gabaritoID !== '') {
                                status = acertou ? 'ACERTOU' : 'ERROU';
                            }
                        } else {
                            // Regra geral
...
async function obterStatusDosPaises() {
    const { data, error } = await supabaseClient
        ...
        .select('pais_id, eliminado, fase_id');
    
    // Objeto padrão de segurança
    const padrao = { eliminados: [], finalistas: [], disputa34: [] };
    ...
    return {
        ...
        finalistas: data.filter(i => parseInt(i.fase_id) === 7).map(i => parseInt(i.pais_id)),
        disputa34: data.filter(i => parseInt(i.fase_id) === 6).map(i => parseInt(i.pais_id))
    };
}
